...
def check_dependencies():
    ...
    if missing:
        print(f"[PKG] Installing missing dependencies: {', '.join(missing)}...")
        try:
...
def check_node_environment():
    ...
    try:
        ...
        print("[ERROR] Error: Node.js is not installed. Please install it from https://nodejs.org/")
        sys.exit(1)

    # 2. Check for node_modules
    if not os.path.exists("node_modules"):
        print("[PKG] 'node_modules' missing. Installing Node.js dependencies...")
        try:
...
def main():
    ...
    if not passcode:
        ...
        print(f"[WARN]  No APP_PASSWORD in .env. Using temporary: {passcode}")
    ...
    try:
        ...
        if node_process.poll() is not None:
            print("[ERROR] Server failed to start immediately. Check server_log.txt.")
            sys.exit(1)
    ...
    try:
        if args.mode == 'local':
            ...
            print("\n" + "="*50)
            print(f"[WIFI] LOCAL WIFI ACCESS")
            print("="*50)
            print(f"[LINK] URL: {final_url}")
            print(f"[KEY] Passcode: Not required for local WiFi (Auto-detected)")
            
            print("\n[PHONE] Scan this QR Code to connect:")
            print_qr(final_url)

            print("-" * 50)
            print("[NOTE] Steps to Connect:")
            ...
            if provider == 'pinggy':
                ...
                try:
                    ...
                    if not public_url:
                        print("[ERROR] Failed to obtain a Pinggy URL.")
                        sys.exit(1)
                except ImportError:
                    print("[ERROR] Error: 'pinggy' package not found. Run: pip install pinggy")
                    sys.exit(1)
                except Exception as e:
                    print(f"[ERROR] Failed to start Pinggy Tunnel: {e}")
                    sys.exit(1)
                ...
                if cf_name:
                    ...
                    if not public_url:
                        print("[WARN]  Warning: CLOUDFLARE_TUNNEL_URL not set. QR Code might be incorrect.")
                    ...
                    try:
                        ...
                        print("[ERROR] Error: 'cloudflared' binary not found.")
                        sys.exit(1)
                else:
                    ...
                    try:
                        ...
                        if not public_url:
                            print("[ERROR] Failed to find Cloudflare Tunnel URL. Is 'cloudflared' installed?")
                            cf_process.terminate()
                            sys.exit(1)
                            
                    except FileNotFoundError:
                        print("[ERROR] Error: 'cloudflared' binary not found. Please install it from: https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/install-run/")
                        sys.exit(1)
                    except Exception as e:
                ...
                if token:
                    ...
                    print("[WARN]  Warning: NGROK_AUTHTOKEN not found in .env. Tunnel might expire.")
            ...
            print("="*50)
            print(f"[LINK] Base URL: {public_url}")
            print(f"[KEY] Passcode: {passcode}")
            
            print("\n[PHONE] Scan this Magic QR Code (Auto-Logins):")
            print_qr(final_url)
        ...
        print("="*50)
        print("[OK] Server is running in background. Logs -> server_log.txt")
        print("[KB]  Press Ctrl+C to stop.")
                    ...
                    with open("server_log.txt", "r", encoding='utf-8', errors='ignore') as f:
                        ...
                        for line in new_lines:
                            if "CDP not found" in line and not cdp_warning_shown:
                                print("\n" + "!"*50)
                                print("[ERROR] ERROR: Antigravity Editor Not Detected!")
                                print("!"*50)

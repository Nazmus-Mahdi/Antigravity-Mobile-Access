...
chatContainer.addEventListener('click', async (e) => {
    ...
    // --- Command Action Buttons and Form Inputs ---
    const interactive = e.target.closest('button, [role="button"], input[type="radio"], label, [role="radio"]');
    if (interactive) {
        const text = (interactive.innerText || interactive.textContent || '').trim();
        const lowerText = text.toLowerCase();

        // Keywords that identify actionable items
        const actionKeywords = [
            'allow', 'always', 'once', 'review', 'confirm', 'accept', 'reject', 
            'discard', 'deny', 'apply', 'save', 'run', 'yes', 'no', 'submit', 'skip'
        ];

        // If it's a radio input, it might not have text, but we still want to click it.
        // Otherwise, it must match a keyword.
        const isInput = interactive.tagName.toLowerCase() === 'input';
        const hasKeyword = actionKeywords.some(kw => lowerText.includes(kw));

        if (isInput || hasKeyword) {
            interactive.style.opacity = '0.5';
            setTimeout(() => interactive.style.opacity = '1', 300);

            // Calculate a robust CSS path from the element up to the chat container
            let current = interactive;
            let path = '';
            while (current && current !== chatContainer && current !== document.body) {
                let nodeName = current.nodeName.toLowerCase();
                let parent = current.parentElement;
                
                if (parent) {
                    let siblings = Array.from(parent.children).filter(el => el.nodeName.toLowerCase() === nodeName);
            ...
            const textContent = (interactive.innerText || interactive.textContent || '').trim();

            console.log('Sending remote click:', { selector: path, text: textContent });
            ...
            try {
                ...
                console.error('Remote interactive click failed:', err);
            }
        }
    }
});

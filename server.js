...
async function captureSnapshot(cdp) {
    const CAPTURE_SCRIPT = `(async () => {
        ...
        try {
            ...
            const redundantElements = clone.querySelectorAll('[contenteditable="true"], [data-lexical-editor], [role="textbox"]');
    ...
    const EXP = `(async () => {
        try {
            ...
            let visibleDialog = Array.from(document.querySelectorAll('[role="dialog"]'))
                                    .find(d => d.textContent?.includes('${mode}') && d.offsetHeight > 0);
            ...
            if (!visibleDialog) {
                // Maybe it's not role=dialog? Look for a popover-like div
                 visibleDialog = Array.from(document.querySelectorAll('div'))
                    .find(d => {
                        if (!d.textContent?.includes('${mode}') || d.textContent?.includes('Files With Changes')) return false; // Anti-context menu
...
async function clickElement(cdp, { selector, index, textContent, text, isExactPath }) {
    // app.js used to send textContent, now it sends text. Handle both.
    const actualText = textContent || text || '';
    const safeText = JSON.stringify(actualText);
    ...
    const EXP = `(async () => {
        try {
            ...
            if (!target) {
                ...
                if (!target) {
                    // Try fallback: querySelectorAll without container
            ...
            if (target) {
                ...
                return { success: true, isExactPath: ${isExactPath ? 'true' : 'false'} };
            }
...
async function setModel(cdp, modelName) {
    const EXP = `(async () => {
        try {
            ...
            visibleDialog = dialogs.find(d => d.textContent?.includes('${modelName}') && d.offsetHeight > 0);
            
            // Fallback: look for positioned divs
            if (!visibleDialog) {
                visibleDialog = Array.from(document.querySelectorAll('div'))
                    .find(d => {
                        if (!d.textContent?.includes('${modelName}') || d.textContent?.includes('Files With Changes')) return false;
                        const style = window.getComputedStyle(d);
                        return d.offsetHeight > 0 && 
                               (style.position === 'absolute' || style.position === 'fixed');
                    });
            ...
            if (!visibleDialog) {
                ...
                const target = allElements.find(el => {
                    ...
                    return (text === '${modelName}' || text?.includes('${modelName}')) && el.offsetParent !== null;
                });
                if (target) {
                    target.click();
...
async function main() {
    ...
    try {
        ...
        app.post('/remote-click', async (req, res) => {
            const { selector, index, textContent, text, isExactPath } = req.body;
            if (!cdpConnection) return res.status(503).json({ error: 'CDP disconnected' });
            const result = await clickElement(cdpConnection, { selector, index, textContent, text, isExactPath });
            res.json(result);
        });
